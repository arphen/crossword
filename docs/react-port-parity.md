...
The React build (`vite build`, `base: './'`) is served by Flask itself at
`/static/react/index.html`; the 15-checkpoint desktop comparison and the
movement trace were rerun against that Flask-served page with
`REACT_PARITY_URL=http://127.0.0.1:5001/static/react/index.html` and pass
identically to the Vite dev server (`base` only affects builds). The original
`base: '/'` build referenced `/assets/...`, which Flask does not serve; the
bundle 404 is what a pre-fix probe found.
...
REACT_PARITY_URL=http://127.0.0.1:5001/static/react/index.html node scripts/react-browser-parity.mjs # against the Flask-served build
