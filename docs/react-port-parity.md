...
relative Vite build base. The current daily-driver entrypoints are `/` and
`/mobile/<room>/<role>`; build assets use `/static/react/` so nested mobile
routes resolve the same bundle. Use `make build` (or `make run`, which builds
first), rather than the old static-index URL. The original `/assets/...`
bundle 404 was fixed before the earlier comparisons; those historical results
do not by themselves verify the new default routes.
