...
Status: **React is now the private daily driver with the Flask/Socket.IO backend.**
`make run` builds the shared legacy assets and React, then serves React at `/`
and `/mobile/<room>/<role>` on port `5001`. Vue remains the fallback reference
at `/legacy/` and `/legacy/mobile/<room>/<role>`. The future public,
generator-backed frontend is not ready; this private app is not that deployment.

The verification results below record the earlier parity runs, not a fresh
certification of every route after the default switch. Nothing is claimed
beyond what the referenced, reproducible commands produced.
...
Command: `node scripts/react-browser-parity.mjs` (now defaults to Flask on
:5001: Vue at `/legacy/`, React at `/`; the earlier runs also used Vite on
:5174). Provider routes are intercepted with a synthetic 15×15 fixture, so no
...
Uses each real mobile route (`/mobile/<room>/<role>` for React,
`/legacy/mobile/<room>/<role>` for Vue), creates isolated in-memory rooms via
`/api/multiplayer/create` on the origin (never under `/legacy/`), and receives
initial state through real Socket.IO joins. Only puzzle HTTP data
...
- Desktop comparison: 15/15 (Vite and Flask-served build); movement: 11/11
  (Vite and Flask-served build); Jest: 11/11; Vitest: 22/22; production
  React build passed (322.42 kB, 101.12 kB gzip).

The earlier Flask-served comparison used `/static/react/index.html` with a
relative Vite build base (`base: './'`); that URL no longer serves the app.
The current daily-driver entrypoints are `/` and `/mobile/<room>/<role>`,
served by Flask from the built bundle, whose hashed assets load from
`/assets/...`. Use `make build` (or `make run`, which builds first), rather
than the old static-index URL. The original `/assets/...` bundle 404 was
fixed before the earlier comparisons; those historical results do not by
themselves verify the new default routes.
...
- **Content gate**: the private React source and Vite proxy now have explicit,
  scoped exemptions in `scripts/forbidden-content.json`, like the private
  continuity surface described in `docs/content-scan.md`. The earlier eight
  violations predated those exemptions. This policy does not make the app a
  public-release artifact; the future generator-backed frontend must exclude
  provider integration and pass its release scan without these exemptions.

## Reproduce

```bash
make run                                      # builds both; Flask React /, Vue /legacy/ on :5001
# Run the following in another terminal:
npm test --workspace @crossword/react-port      # behavior + differential suites
node scripts/react-browser-parity.mjs           # 15-checkpoint comparison of Flask-served pages
PARITY_TRACE=movement node scripts/react-browser-parity.mjs # 11 movement states
node scripts/grid-click-browser-check.mjs       # click/arrow selection + watermark colors
node scripts/watermark-browser-check.mjs        # defaults to Vue /legacy/
node scripts/mobile-browser-parity.mjs          # 10 mobile viewport states
```

To compare the optional Vite dev server instead, start
`npm run dev --workspace @crossword/react-port` alongside Flask and set
`REACT_PARITY_URL=http://127.0.0.1:5174/`. `VUE_PARITY_URL` defaults to
`http://127.0.0.1:5001/legacy/`. The mobile harness uses each URL's origin for
API creation and applies the legacy prefix only to Vue page navigation.
